...
public class RuleTest {
  ...
  public void testValidate() throws DatalogException {
    ...
    Rule rule = new Rule(new Expr("p", "A", "B"), new Expr("q", "A"), new Expr("q", "B"),
        new Expr("<>", "A", "B"));
    rule.validate();
    assertTrue(true);

    rule = new Rule(new Expr("p", "A", "B"), new Expr("q", "A"), new Expr("q", "B"),
        new Expr("q", "C"), new Expr("=", "C", "B"));
    rule.validate();
    assertTrue(true);

    try {
      // The variable C must appear in the body - exception thrown
      rule = new Rule(new Expr("p", "A", "C"), new Expr("q", "A"), new Expr("q", "B"),
          new Expr("<>", "A", "B"));
      rule.validate();
      assertFalse(true);
  ...
  public void testSubstitute() throws DatalogException {
    ...
    bindings.put("A", new Term("aa"));
    Rule subsRule = rule.substitute(bindings);
    assertTrue(subsRule.equals(new Rule(new Expr("p", "aa", "B"), new Expr("q", "aa"),
        new Expr("q", "B"), new Expr("<>", "aa", "B"))));

    // Original rule unchanged?
    assertTrue(rule.equals(new Rule(new Expr("p", "A", "B"), new Expr("q", "A"), new Expr("q", "B"),
        new Expr("<>", "A", "B"))));
  }
}
